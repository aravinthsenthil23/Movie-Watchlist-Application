...
console.log(movies_lists)

function reRender() {
    movies_container.innerHTML = ""
    for (const imdbID of movies_lists) {
        moviesList(imdbID)
    }
    if (movies_lists.length === 0) {
        document.getElementById("watchlist-container").style.display = "flex"
        movies_container.style.display = "none"
    }
    else if (movies_lists.length > 0) {
...
async function moviesList(imdbID) {
    movies_container.style.setProperty("background-image", 'none')
    const res = await fetch(`https://www.omdbapi.com?apikey=574e9abd&i=${imdbID}`)
